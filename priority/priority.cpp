...
public:
  explicit Verbose( std::string d, std::string s = "stop" )
    : desc(std::move(d))
    , stop(std::move(s))
  {
...
public:
  explicit priority_props( boost::fibers::context* ctx )
    :
...
//->
  ...
  [[nodiscard]] bool has_ready_fibers() const noexcept override
  {
  ...
  void property_change( boost::fibers::context* ctx, priority_props& props ) noexcept override
  {
...
boost::fibers::fiber launch( Fn&& func, std::string const& name, int priority )
{
  ...
  auto& props(fiber.properties<priority_props>());
  props.name = name;
...
//->
  ...
  auto& other_props(other.properties<priority_props>());
//<-
...
//]
...

  {
    ...
    boost::fibers::fiber   low(launch([&barrier]()
                                      { barrier_fn(barrier); }, "low", 1));
    boost::fibers::fiber   med(launch([&barrier]()
                                      { barrier_fn(barrier); }, "medium", 2));
    boost::fibers::fiber   hi(launch([&barrier]()
                                     { barrier_fn(barrier); }, "high", 3));
    std::cout << "main: low.join()" << std::endl;
    low.join();
    std::cout << "main: medium.join()" << std::endl;
    med.join();
...

  {
    ...
    boost::fibers::barrier barrier(3);
    boost::fibers::fiber   c(launch([&barrier]()
                                    { barrier_fn(barrier); }, "c", 1));
    boost::fibers::fiber   a(launch([&c, &barrier]()
                                    { change_fn(c, 3, barrier); }, "a", 3));
    boost::fibers::fiber   b(launch([&barrier]()
                                    { barrier_fn(barrier); }, "b", 2));
    std::cout << "main: a.join()" << std::endl;
    std::cout << "main: a.join()" << std::endl;
    a.join();
    std::cout << "main: b.join()" << std::endl;
