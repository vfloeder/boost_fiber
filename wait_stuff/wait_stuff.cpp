...
struct Example {
  Example( Runner& runner2use, std::string const& desc, std::function<void()> const& func )
  {
    runner2use.add(desc, func);
  }
};
...
// immediately.
void wait_first_simple_impl( const Done::ptr& )
{
}
...
public:
  explicit exception_list( std::string const& what )
    :
  ...
  void add( const std::exception_ptr& ep )
  {
...
// Degenerate case: when there are no functions to wait for, return
// immediately.
void wait_all_simple_impl( const std::shared_ptr<boost::fibers::barrier>& )
{
}
...
class nchannel {
public:
  nchannel( std::shared_ptr<boost::fibers::buffered_channel<T>> chan, std::size_t lm )
    : chan_(std::move(chan))
    , limit_(lm)
  {
    assert(chan_);
